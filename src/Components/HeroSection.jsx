...
const HeroSection = ({ scrollTo, isDarkMode }) => {
  const [history, setHistory] = useState([
    {
      cmd: "",
      output:
        'Welcome to Kinetic Env v1.0.0\n[ Interactive Terminal: Type your commands below ]\nType "help" to see available commands.',
    },
  ]);
  const [input, setInput] = useState("");
  const [showDocs, setShowDocs] = useState(false);
  ...
  // State for command history navigation (Up/Down arrows)
  ...
  const handleCommand = async (e) => {
    ...
    if (e.key === "Enter") {
      ...
      if (validSections.includes(cmd)) {
        ...
        setHistory((prev) => [
          ...prev,
          { cmd: rawCmd, output: "Pinging API uplink..." },
        ]);
        setInput("");
        try {
          const startTime = performance.now();
          const res = await fetch(`${API_BASE_URL}api/v1/skills/`); // Lightest endpoint to check
          ...
          if (res.ok) {
            setHistory((prev) => [
              ...
              {
                cmd: "",
                output: `[OK] API Connection established. Latency: ${pingTime}ms`,
              },
            ]);
          } else {
            setHistory((prev) => [
              ...
              {
                cmd: "",
                output: `[ERROR] API responded with status ${res.status}`,
              },
            ]);
          }
        } catch (error) {
          setHistory((prev) => [
            ...prev,
            {
              cmd: "",
              output:
                "[FAILED] API is currently unreachable. Offline mode active.",
            },
          ]);
        }
        return; // Return early because we updated history asynchronously
      } else {
        switch (cmd) {
          case "help":
            output =
              "Available commands:\n  whoami     - Print user information\n  skills     - List technical proficiencies\n  details    - View personal data\n  [section]  - Type any section (e.g., education, projects) to navigate\n  email      - Output contact email\n  github     - Display GitHub profile link\n  linkedin   - Display LinkedIn profile link\n  twitter    - Display Twitter profile link\n  neofetch   - Display system information\n  date       - Print current date and time\n  ping       - Check API uplink status\n  reboot     - Restart the kinetic environment\n  clear      - Clear terminal output";
            break;
          case "whoami":
            output = `${profileData.name}\n${profileData.role} | ${profileData.subRole}`;
            break;
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-1 gap-3">
                    {[
                      ...
                      {
                        cmd: "ping",
                        desc: "Check API uplink status and latency.",
                      },
                      {
                        cmd: "reboot",
                        desc: "Restart the kinetic environment.",
